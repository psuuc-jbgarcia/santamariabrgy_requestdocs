...
if ($updateResult !== false && $updateResult->affected_rows > 0) {
    if ($newStatus === 'Ready to Pick Up') {
        ...
        $stmt = "SELECT email FROM permits WHERE tracking_code=?";
        ...
        if ($selectResult->num_rows > 0) {
            ...
            $mail->Body = 'Your Business Permit document is now ready for pickup. Please visit our office to collect it. Additionally, please note that there is a payment of 100 pesos required for this service.';
...
} elseif ($newStatus === 'Ready to Pick Up') {
    // Error updating status to "Ready to Pick Up"
    $response = array(
        'title' => 'Error',
        'icon' => 'error',
        'text' => 'Error updating status to "Ready to Pick Up": ' . $conn->error
    );
} else {
    // Error updating status for other cases
    $response = array(
        'title' => 'Error',
        'icon' => 'error',
